...
public class ComputeShaderMonoBehaviourScript : MonoBehaviour
{
    ...
    void Start()
    {
        int x = 8;
        int y = 8;
        ComputeBuffer computeBuffer = new ComputeBuffer(4 * x * y, sizeof(float));//RWstructuredBuffer‚ÅÀÛ‚É‚Â‚©‚¤‚â‚Â

        int kernelHandle = computeShader.FindKernel("CSMain");
        computeShader.SetBuffer(kernelHandle, "Result", computeBuffer);
        computeShader.Dispatch(kernelHandle, x / 8, y / 8, 1);
